...
def main():
    ...
    products = [
            ...
            {"name": "Monterey (12.6)",     "b": "Mac-B809C3757DA9BB8D", "m": "00000000000000000", "os_type": "latest", "short": "monterey",    "ver": "12"},
    ...
    answer = input('\nChoose a macOS version (1-%s): ' % len(products))
    index = int(answer) - 1
    if index < 0:
        raise ValueError
    ...
    # Convert .dmg to .img
    ############################################
    print("\nConverting .dmg to .img file...")
    os.system(f'dmg2img "{dmg_file}"')
    os.unlink(dmg_file)
    ...
    ############################################
    # Copy empty but pre-partioned/pre-formatted disk file into machine's folder
    ############################################
